...
async def on_ready ():
	...
	await client.change_presence( status = discord.Status.online, activity = discord.Activity( name = '..help for help', type=discord.ActivityType.watching) )
...
async def prefix(ctx, arg: str = None):
    if arg is None:
        ...
        prefixes.update_one({"_guild_id": ctx.guild.id}, {"$set": {"prefix": arg}})
